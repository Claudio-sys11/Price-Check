...
3. **가격비교 탭 → [Wizfasta 원가 가져오기]**:
   - **백그라운드(headless) Chrome**으로 자동 로그인(창 안 보임) → 상품DB **일반상품·재고≥1·500개씩** 조회 → **전체 엑셀 다운로드** 후 모델명·원가 추출 (엑셀 실패 시 그리드 폴백)
   - **진행 체크리스트**(Chrome 시작 → 로그인 → 조회 → 엑셀 다운로드 → 파싱 → 매칭)가 하나씩 ✅ 로 완료 표시됩니다

> Wizfasta 로그인 정보는 `%APPDATA%\EcountInventory\config.json` 에 저장됩니다(로컬).
