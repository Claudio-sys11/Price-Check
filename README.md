...
2. **[설정] → 'Wizfasta 로그인'에 업체코드·아이디·비밀번호 입력** (1회)
3. **가격비교 탭 → [Wizfasta 원가 가져오기]**:
   - **Chrome 창을 열어** 로그인 화면으로 이동 → 자동 로그인 → 상품DB **일반상품·재고≥1·500개씩** 조회 → **전체 엑셀 다운로드** 후 모델명·원가 추출 (엑셀 실패 시 그리드 폴백)
   - **진행 체크리스트**(Chrome 시작 → 로그인 → 조회 → 엑셀 다운로드 → 파싱 → 매칭)가 ✅·⏳(스피너+경과초)로 실시간 표시
   - **Chrome 시작이 60초 내 안 되면 종료**되고, 버튼을 다시 눌러 재시도할 수 있습니다

> Wizfasta 로그인 정보는 `config.json` 에 저장(로컬). **미입력 시 공백**이며, **저장 후 24시간 지나면 자동 공백 초기화**(설정의 '24시간 이후에도 계속 저장' 체크 시 유지).

## 설치 현황 탭 (③)

이 프로그램 사용에 필요한 항목의 설치 상태를 확인하고 링크로 설치합니다:
- **Google Chrome** — 설치 여부 표시 + [Chrome 설치/다운로드] 링크
- **chromedriver** — 앱이 자동 설치·관리(별도 설치 불필요)
- **이 프로그램** — 현재 버전 + [최신 릴리스 페이지] 링크
