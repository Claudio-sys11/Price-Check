...
GUI 사용법:
1. 앱 실행 → 회사코드 / 사용자ID / API 인증키 입력 (환경: production/test 선택)
2. (선택) 기준일자·품목코드·창고코드 입력
3. **재고현황 조회** 클릭 → 표로 결과 표시
4. **CSV 내보내기** 로 저장
...
재고현황 조회 시 EcountERP **품목명**(`브랜드_모델명_사이즈_입고일자`)을 `_` 기준으로 분해해 표시합니다.
...
표시 컬럼: **브랜드 / 모델명 / 사이즈 / 입고일자 / 품목코드 / 품목명 / 재고수량**

- 재고수량은 **소수점을 제외한 정수**로 표시합니다.
- 표의 모든 셀·헤더는 **가운데 정렬**입니다.

> ⚠️ 품목명은 재고현황 API에 없고 **품목 조회 API(`GetBasicProductsList`)** 로 가져옵니다.
> 이 API 권한이 인증키에 없으면 재고수량·품목코드만 표시되고 분해 컬럼은 비어 있습니다(권한 활성화 시 자동 채움).
